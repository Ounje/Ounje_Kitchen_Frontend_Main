...
export default function NotificationItem({ notification }: NotificationItemProps) {
  ...
  return (
    <div
      className={`flex items-start gap-3 p-3 rounded-lg transition-colors ${isUnread ? "bg-white" : "bg-gray-50/60"}`}
    >
      ...
      <div className="relative shrink-0">
        ...
        <span
          className={`absolute -bottom-0.5 -right-0.5 w-4.5 h-4.5 flex items-center justify-center rounded-full ${config.iconBg} ring-2 ring-white`}
        >
      ...
      <div className="flex-1 min-w-0 space-y-0.5">
        <div className="flex items-center gap-1.5">
          ...
          <p
            className={`text-xs leading-snug line-clamp-1 ${isUnread ? "font-semibold text-gray-900" : "font-medium text-gray-600"}`}
          >
            {notification.title}
          </p>
        </div>
