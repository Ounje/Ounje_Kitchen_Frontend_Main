...
export function ConfirmActionModal({ isOpen, title = 'Are you sure?', loading = false, onConfirm, onClose }: ConfirmActionModalProps) {
      ...
      <div className="w-full max-w-sm rounded-2xl p-8 shadow-2xl space-y-6 text-center"
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={loading}
            ...
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Yes, Confirm'}
          </button>
        </div>
      </div>
    </div>
