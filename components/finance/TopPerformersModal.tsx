  ...
  return (
    <div className="px-6 pt-6 pb-4">
      <div className="flex items-start justify-between mb-1">
        <div>
          <h2 className="text-xl font-bold" style={{ color: '#1A3F1C' }}>{title}</h2>
...
export function TopVendorsModal({ isOpen, onClose, vendors, period, onPeriodChange, loading }: TopVendorsModalProps) {
          ...
          <table className="w-full min-w-[540px] rounded-xl overflow-hidden">
            ...
            <tbody className="bg-white">
              {loading ? (
                ...
                <tr key={v.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i === 0 ? 'border-t-0' : ''}`}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium" style={{ color: '#1A3F1C' }}>{v.name}</span>
                    </div>
                  </td>
...
export function TopRidersModal({ isOpen, onClose, riders, period, onPeriodChange, loading }: TopRidersModalProps) {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div
                ...
                <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium" style={{ color: '#1A3F1C' }}>{r.name}</span>
                    </div>
                  </td>
