...
export function StatsCard({ icon, value, label, subtitle, change }: StatsCardProps) {
  return (
    <div
      className="rounded-xl p-4 sm:p-5 flex flex-col gap-2 relative overflow-hidden bg-[#98ef9b] dark:bg-primary/20"
    >
      ...
      <div className="flex items-center justify-between">
        {subtitle && (
          <p className="text-xs text-[#1A3F1C] dark:text-primary/70">{subtitle}</p>
        )}
        {change !== undefined && (
          <span
            className="text-xs font-semibold flex items-center gap-0.5 ml-auto text-[#1A3F1C] dark:text-primary"
          >
