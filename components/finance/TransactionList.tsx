...
function PaymentBadge({ method }: { method: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 rounded text-[10px] font-semibold border ${cls} whitespace-nowrap`}
    >
      {method || "—"}
    </span>
  );
...
export function TransactionList({ groups, onInfo, onPrint }: Props) {
  ...
  // Pre-compute sequential row numbers to avoid mutation during render
