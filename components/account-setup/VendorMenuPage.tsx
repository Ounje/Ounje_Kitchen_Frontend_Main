...
function getFoodItemPriceRange(item: any): string {
  const prices: number[] = (item.subCategory ?? []).flatMap((sc: any) =>
    (sc.items ?? []).map((i: any) => Number(i.price)).filter(Boolean)
  );
...
function FoodItemViewModal({
  ...
  const saveEdit = (si: number, di: number) => {
    const updated = subCategories.map((sc, s) =>
      s !== si
        ? sc
        : {
            ...sc,
            items: sc.items.map((dish: any, d: number) =>
              d !== di
                ? dish
                : {
                    ...dish,
      ...
      <DialogContent className="w-[calc(100%-1.5rem)] sm:max-w-lg p-0">
        <DialogHeader className="px-4 sm:px-6 pt-5 pb-0">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Click the pencil icon to edit a dish&apos;s name, price, or image.
          </p>
        </DialogHeader>
                ...
                <div key={si}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-gray-400">
                      {sc.items?.length ?? 0} dish{sc.items?.length !== 1 ? "es" : ""}
                    </span>
                  </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2 justify-end">
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="h-7 gap-1 text-gray-400"
                                  onClick={cancelEdit}
                                >
                                ...
                                <Button
                                  size="sm"
                                  className="h-7 gap-1 bg-[#1a3f1c] hover:bg-[#1a3f1c]/90"
                                  onClick={() => saveEdit(si, di)}
                                >
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  <span
                                    className={`text-[9px] font-black uppercase ${dish.isAvailable !== false ? "text-green-600" : "text-gray-400"}`}
                                  >
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        {(
          ...
          ] as { key: MenuTab; label: string; icon: React.ReactNode }[]
        ).map((t) => (
          <button
            key={t.key}
            type="button"
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-2">
            ...
            <p className="text-sm font-medium">
              No {tab === "food-items" ? "food categories" : "combos"} yet
            </p>
