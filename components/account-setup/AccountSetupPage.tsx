    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Create and manage customer, vendor, and rider accounts</p>
        </div>
        <Button
          onClick={() => { setEditTarget(null); setShowForm(true); }}
          className="bg-[#1a3f1c] hover:bg-[#1a3f1c]/90 gap-2"
        >
          <Plus className="w-4 h-4" /> Create {tab === "customers" ? "Customer" : tab === "vendors" ? "Vendor" : "Rider"}
        </Button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-2">
            {tab === "customers" ? <Users className="w-10 h-10 opacity-20" /> :
             tab === "vendors"   ? <Store className="w-10 h-10 opacity-20" /> :
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-100">
                  ...
                  <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">Phone</th>
                ...
                {items.map((item: any) => {
                  const name  = tab === "customers" ? item.name :
                                tab === "vendors"   ? item.name :
                                                      item.user?.name ?? "—";
                  const email = tab === "customers" ? item.email :
                                tab === "vendors"   ? item.owner?.email ?? "—" :
                                                      item.user?.email ?? "—";
                  const phone = tab === "customers" ? item.phone ?? "—" :
                                tab === "vendors"   ? item.owner?.phone ?? "—" :
                                                      item.user?.phone ?? "—";
                  const isActive = tab === "customers" ? item.profile?.isActive ?? item.isActive :
                                   tab === "vendors"   ? item.isActive :
                                                         item.isActive;
                  return (
        ...
        <Pagination
          ...
          onPageSizeChange={s => { setPageSize(s); setPage(1); fetchData(1, s, search); }}
        />
      </div>
