...
import {
  ArrowRight, Bell, CheckCircle, Trash2, X,
...
function PortalBadge({ name }: { name: string }) {
  const colors = portalColors[name] || { bg: 'bg-gray-100', text: 'text-gray-700', border: 'border-gray-200' };
  return (
...
const typeMeta: Record<string, { label: string; bg: string; text: string; icon: any }> = {
  promo_approval_request: { label: 'Needs Approval',  bg: 'bg-amber-50',  text: 'text-amber-700',  icon: AlertCircle },
  promo_approved:         { label: 'Approved',         bg: 'bg-green-50',  text: 'text-green-700',  icon: CheckCircle },
  promo_declined:         { label: 'Declined',         bg: 'bg-red-50',    text: 'text-red-700',    icon: X },
  promo_approval_processed: { label: 'Processed',       bg: 'bg-gray-50',   text: 'text-gray-600',   icon: CheckCircle },
...
export function NotificationDetailModal({ notification, open, onClose }: NotificationDetailModalProps) {
  const { user } = useAuth();
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Marked as read'); invalidate(); onClose(); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Notification deleted'); invalidate(); onClose(); },
  ...
  const isPromoApprovalRequest = notification?.type === 'promo_approval_request' || notification?.type === 'promo_approval_processed';
  ...
  const canApprove = isPromoApprovalRequest && user?.isSuperAdmin && notification?.relatedId && !isLoadingPromo && isPending;

  if (!notification) return null;

  const fromPortal = notification.sourcePortal || 'System';
  const toPortal   = notification.targetPortal || notification.audience || 'All';
  const formattedDate = notification.createdAt ? new Date(notification.createdAt).toLocaleString() : 'Just now';
        ...
        <div className="px-6 py-5 space-y-4 bg-white">
          ...
          {isPromoApprovalRequest && promoData && (
            <div className="bg-white border rounded-lg overflow-hidden shadow-sm">
              <div className="bg-gray-50 px-3 py-1.5 border-b text-[10px] font-bold text-gray-500 uppercase tracking-wider">Promo Details</div>
              <div className="p-3 grid grid-cols-2 gap-y-2 gap-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold">
                    {promoData.discountValue}{promoData.discountType === 'percentage' ? '%' : '₦'}
                  </p>
                </div>
                <div>
          ...
          {isPromoApprovalRequest && promoData && !isPending && (
            <div className={`rounded-lg p-3 flex items-center gap-2 border ${
              ...
              <p className="text-sm font-semibold">
                ...
                <span className="capitalize ml-1">{promoData.status === 'active' ? 'Approved' : promoData.status}</span>
              </p>
            </div>
          )}
          ...
          {canApprove && !showDeclineForm && (
            <div className="border border-amber-200 bg-amber-50 rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-amber-800">This promo requires your approval</p>
              </div>
          ...
          {showDeclineForm && (
            <div className="border-2 border-red-100 bg-red-50/50 rounded-xl p-5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="space-y-2">
                <Label htmlFor="decline-reason" className="text-xs font-bold text-red-800 uppercase tracking-wider flex items-center gap-1">
