...
import { Home, ArrowLeftRight, ArrowDownToLine, TrendingUp, Settings, LogOut, X, Wallet, ShieldCheck } from "lucide-react";
...
const NAV = [
  ...
  { href: "/finance/payouts",         name: "Payouts",         icon: Wallet          },
...
export default function FinanceSidebar({ isOpen, onClose }: FinanceSidebarProps) {
  ...
  const fullName  = [user?.firstName, user?.lastName].filter(Boolean).join(" ") || "Finance";
