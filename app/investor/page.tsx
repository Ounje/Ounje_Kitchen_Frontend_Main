...
import {
  TrendingUp, ShoppingCart, Store, Users, Bike,
  DollarSign, CheckCircle, BarChart2, ChevronDown,
} from 'lucide-react';
import investorService, {
  type InvestorDashboard, type InvestorRevenue, type InvestorPeriod,
...
function KpiCard({
  ...
  return (
    <div className={`rounded-2xl p-5 border ${
      ...
      <p className={`text-2xl font-black leading-none mb-1 ${accent ? 'text-white' : 'text-gray-900'}`}>
        {value}
      </p>
      <p className={`text-xs font-semibold ${accent ? 'text-white/70' : 'text-gray-500'}`}>{label}</p>
...
export default function InvestorPage() {
  ...
  useEffect(() => {
    setRevLoading(true);
    investorService.getRevenue(period)
      .then(setRevenue)
      .catch(() => {})
  ...
  return (
    <div className="p-6 lg:p-10 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown className={`w-4 h-4 transition-transform ${showPeriod ? 'rotate-180' : ''}`} />
          </button>
          {showPeriod && (
            <div className="absolute right-0 mt-1 w-36 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-10">
              {PERIODS.map(p => (
                <button key={p} type="button" onClick={() => { setPeriod(p); setShowPeriod(false); }}
                  className={`w-full text-left px-4 py-2 text-sm transition-colors ${
                    period === p ? 'bg-[#98ef9b]/30 text-[#1a3f1c] font-bold' : 'text-gray-600 hover:bg-gray-50'
                  }`}>
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard icon={DollarSign}   label="All-Time Revenue"     value={fmt(stats.totalRevenue)}    accent />
          <KpiCard icon={TrendingUp}   label="This Month Revenue"   value={fmt(stats.monthlyRevenue)}
            sub={stats.revenueGrowth >= 0 ? `↑ +${stats.revenueGrowth}% vs last month` : `↓ ${stats.revenueGrowth}% vs last month`} />
          <KpiCard icon={ShoppingCart} label="Total Orders"         value={stats.totalOrders.toLocaleString()} />
          <KpiCard icon={CheckCircle}  label="Completion Rate"      value={`${stats.completionRate}%`}
            sub={`${stats.deliveredOrders.toLocaleString()} delivered`} />
          <KpiCard icon={Store}        label="Active Vendors"       value={stats.activeVendors.toLocaleString()} />
          <KpiCard icon={Users}        label="Total Customers"      value={stats.totalCustomers.toLocaleString()} />
          <KpiCard icon={Bike}         label="Active Riders"        value={stats.activeRiders.toLocaleString()} />
          <KpiCard icon={BarChart2}    label="Delivered Orders"     value={stats.deliveredOrders.toLocaleString()} />
        </div>
      )}
