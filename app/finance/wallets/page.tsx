...
function SummaryCard({
  ...
  return (
    <div
      className={`rounded-xl p-4 border ${accent ? "bg-[#98ef9b]/20 border-[#1a3f1c]/20" : "bg-white border-gray-100 shadow-sm"}`}
    >
      <p className="text-xs font-semibold text-gray-500 mb-2">{label}</p>
      <p className={`text-xl font-black ${accent ? "text-[#1a3f1c]" : "text-gray-900"}`}>
        {fmt(available)}
      </p>
      <p className="text-xs text-gray-400 mt-1">Available</p>
      <div className="mt-2 flex flex-wrap gap-3 text-xs text-gray-500">
        <span>
          Pending: <span className="font-semibold text-amber-600">{fmt(pending)}</span>
        </span>
        {hold !== undefined && (
          <span>
            Hold: <span className="font-semibold text-blue-600">{fmt(hold)}</span>
          </span>
        )}
      </div>
    </div>
...
function HistoryModal({
  ...
  const rows: WalletLedgerEntry[] = bankTab ? (history?.bank ?? []) : (history?.ledger ?? []);
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Available:{" "}
              <span className="font-semibold text-[#1a3f1c]">{fmt(item.availableBalance)}</span>
              {item.pendingBalance > 0 && (
                <>
                  {" "}
                  · Pending:{" "}
                  <span className="font-semibold text-amber-600">{fmt(item.pendingBalance)}</span>
                </>
              )}
              {item.holdBalance > 0 && (
                <>
                  {" "}
                  · Hold:{" "}
                  <span className="font-semibold text-blue-600">{fmt(item.holdBalance)}</span>
                </>
              )}
            </p>
          </div>
          ...
          {loading ? (
            ...
            rows.map((e) => {
              ...
              return (
                <div
                  key={e._id}
                  className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50 hover:bg-white transition-colors"
                >
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={`text-sm font-bold tabular-nums ${isCredit ? "text-green-600" : "text-red-500"}`}
                    >
                      {isCredit ? "+" : "-"}
                      {fmt(e.amount)}
                    </p>
...
function BalancesTable({ accountType }: { accountType: WalletAccountType }) {
  ...
  const load = useCallback(
    async (page: number, searchVal: string, limit = pageSize) => {
      setLoading(true);
      try {
  ...
  return (
    <>
      ...
      {/* History modal */}
      {selected && (
        <HistoryModal item={selected} accountType={accountType} onClose={() => setSelected(null)} />
      )}
    </>
  );
}
    ...
    <div className="w-full space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">
          Live wallet balances for every customer, vendor, and rider. Click any row to see their
          transaction history.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {overviewLoading ? (
          ...
          <>
            ...
            />
            <BankCard total={bankTotal?.totalNaira ?? 0} count={bankTotal?.count ?? 0} />
          </>
        )}
      </div>
