  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-14 py-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
          ...
          <div className="flex-1 space-y-3">
            {[64, 48, 56, 72, 80].map((w, i) => (
              <div key={i} className={`h-5 bg-gray-300 rounded animate-pulse w-${w}`} style={{ width: `${w * 4}px` }} />
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[...Array(4)].map((_, i) => <div key={i} className="h-32 bg-gray-300 rounded-xl animate-pulse" />)}
      </div>
