...
export default function RiderDocumentPage() {
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4 border-[#1a3f1c]" />
  ...
  const docType: string =
    (document as any)?.documentType ?? (docUrl.toLowerCase().endsWith(".pdf") ? "pdf" : "image");
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-14 py-6">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1a3f1c]">Document Upload</h1>
          ...
          >
            <X className="w-6 h-6 text-[#1a3f1c]" />
          </button>
        </div>

        {/* Document Card */}
        <div className="bg-white rounded-xl p-4 sm:p-6 w-full">
          <h2 className="text-lg sm:text-xl font-bold mb-4 text-[#1a3f1c]">Rider&apos;s NIN</h2>
          ...
          {error || !hasDoc ? (
            <div className="text-center py-12">
              ...
              <button
                onClick={() => router.back()}
                className="px-6 py-2 rounded-lg text-white bg-[#1a3f1c]"
              >
                Go Back
              </button>
            </div>
