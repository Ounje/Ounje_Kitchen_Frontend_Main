...
function getDisplayName(item: any, view: ViewType): string {
  ...
  if (view === "Rider")
    return (
      item.user?.name || item.name || `${item.firstName ?? ""} ${item.lastName ?? ""}`.trim() || "—"
    );
  return "—";
}
...
function resolveLineManager(lm: any, lookupList: any[]): string {
  ...
  if (typeof lm === "object") {
    return lm.name || `${lm.firstName ?? ""} ${lm.lastName ?? ""}`.trim() || lm.email || "—";
  }
  // Raw ObjectId string — look up in full staff list
  const lmStr = String(lm);
  const found = lookupList.find((a) => String(a._id) === lmStr || String(a.id) === lmStr);
  if (found) {
    return (
      `${found.firstName ?? ""} ${found.lastName ?? ""}`.trim() || found.name || found.email || "—"
    );
  }
  return "—";
}
...
export default function ITAdminPage() {
  ...
  const fetchAllStaffLookup = useCallback(async () => {
    try {
      ...
      /* silent — just means lineManager won't resolve */
    }
  }, []);
  ...
  const fetchData = useCallback(
    async (page = 1, limit = pagination.limit) => {
      ...
      try {
        ...
        switch (currentView) {
          case "Admin":
            res = await itService.getAdmins(base);
            break;
          case "Staff":
            res = await itService.getStaff(base);
            break;
          case "Customer":
            res = await itService.getCustomers(base);
            break;
          case "Vendor":
            res = await itService.getVendors(base);
            break;
          case "Rider":
            res = await itService.getRiders(base);
            break;
        }
          ...
          <CardContent className="pt-6">
            ...
            <form onSubmit={handleCreateAdmin} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  type="submit"
                  className="bg-primary text-white hover:opacity-90 px-8 py-2 h-10 font-bold"
                >
                  Create Admin
                </Button>
                <Button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setStaffForm({ ...staffForm, lineManager: v })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select line manager" />
                    </SelectTrigger>
                    <SelectContent>
                      {allStaffLookup.length === 0 && (
        ...
        <Card className="border shadow-sm">
          <CardContent className="pt-6">
            ...
            <div className="flex gap-2">
              <Button
                onClick={() => fetchData(1)}
                className="bg-primary text-white hover:opacity-90 px-8 font-bold h-10"
              >
                Search
              </Button>
              <Button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-secondary">
                <tr>
                  <th className="px-4 py-4 text-left font-black text-primary uppercase tracking-wider text-[10px]">
                    S/N
                  </th>
                  <th className="px-4 py-4 text-left font-black text-primary uppercase tracking-wider text-[10px]">
                    Name
        ...
        <DialogContent className="sm:max-w-md bg-[#4B8A4E] text-white md:p-8">
          <DialogHeader>
            ...
            <DialogDescription className="text-base pt-4">
              ...
              <br />
              <span className="font-semibold text-white mt-2 block">{deleteTarget?.name}</span>
              <br />
