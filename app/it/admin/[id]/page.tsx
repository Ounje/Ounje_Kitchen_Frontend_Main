...
function Avatar({ src, name, size = 110 }: { src?: string; name: string; size?: number }) {
  ...
  return (
    <div className="rounded-xl flex items-center justify-center flex-shrink-0 border border-gray-200 shadow-sm"
...
function StatBox({ value, label, color }: { value?: number; label: string; color: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl min-w-[72px] min-h-[88px] px-3 py-3 gap-1"
      ...
      <div className="bg-[#e8f7e8] rounded-2xl p-6 space-y-5">
        ...
        <div className="flex gap-5">
          ...
          <div className="flex-1 space-y-3">
            {[1,2,3,4].map(i => <div key={i} className="h-4 bg-gray-200 rounded" style={{ width: `${70 - i*8}%` }} />)}
          </div>
        </div>
        <div className="h-4 bg-gray-300 rounded w-24" />
        <div className="flex gap-3">
          {[1,2,3,4].map(i => <div key={i} className="w-20 h-24 rounded-xl bg-gray-300" />)}
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-md" style={{ backgroundColor: "#e8f7e8" }}>
      ...
      <div className="px-6 pt-6 pb-4 flex items-start justify-between">
        ...
        <span className={`px-5 py-1.5 rounded-lg text-sm font-bold text-white ${isActive ? "bg-[#1a3f1c]" : "bg-red-500"}`}>
      ...
      <div className="px-6 pb-5 flex items-start gap-5">
        ...
        <div className="flex-1 space-y-2 text-[15px]">
          <p><span className="font-bold">Name:</span> <span className="text-gray-700">{name}</span></p>
          <p><span className="font-bold">Phone number:</span> <span className="text-gray-700">{phone}</span></p>
          <p><span className="font-bold">Email:</span> <span className="text-gray-700">{email}</span></p>
          ...
          <div className="flex-1 min-w-0 rounded-xl overflow-hidden bg-white shadow-sm border border-gray-100 flex flex-col">
            ...
            {mv ? (
              <div className="p-3 flex gap-2.5 flex-1 items-center">
                <Avatar src={mv.photo || mv.image || mv.avatar || ""} name={mv.name || "Vendor"} size={50} />
                ...
                <div className="flex flex-col items-center justify-center rounded-lg px-2.5 py-2 flex-shrink-0"
                  style={{ backgroundColor: "#1a3f1c" }}>
                  <span className="text-white text-xl font-extrabold leading-none">{mv.ordersCount ?? 0}</span>
                  <span className="text-white/80 text-center leading-tight mt-0.5" style={{ fontSize: "9px" }}>
                    Orders here
                  </span>
                </div>
              </div>
...
function VendorDetail({ data }: { data: any }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-md" style={{ backgroundColor: "#e8f7e8" }}>
      ...
      <div className="px-6 pb-5 flex items-start gap-5">
        <Avatar src={data.photo || data.image || data.avatar || data.owner?.avatar || ""} name={name} size={110} />
        <div className="flex-1 space-y-2 text-[15px]">
          <p><span className="font-bold">Name:</span> <span className="text-gray-700">{name}</span></p>
          <p><span className="font-bold">Phone number:</span> <span className="text-gray-700">{phone}</span></p>
          ...
          <div className="flex-1 min-w-0 rounded-xl overflow-hidden bg-white shadow-sm border border-gray-100 flex flex-col">
            ...
            {mfb ? (
              <div className="p-3 flex gap-2.5 flex-1 items-center">
                ...
                <div className="flex flex-col items-center justify-center rounded-lg px-2.5 py-2 flex-shrink-0"
                  style={{ backgroundColor: "#1a3f1c" }}>
                  <span className="text-white text-xl font-extrabold leading-none">{mfb.totalOrders ?? 0}</span>
                  <span className="text-white/80 text-center leading-tight mt-0.5" style={{ fontSize: "9px" }}>
                    Orders here
                  </span>
                </div>
...
function RiderDetail({ data }: { data: any }) {
  const name  = data.user?.name
    ...
    ?? NA;
  const phone = data.phoneNumber ?? (data.user?.phone ? String(data.user.phone) : null) ?? (data.phone ? String(data.phone) : NA);
  ...
  const mostFreqZone = typeof data.mostFrequentZone === "string"
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-md" style={{ backgroundColor: "#e8f7e8" }}>
      ...
      <div className="px-6 pb-5 flex items-start gap-5">
        <Avatar src={data.avatar || data.image || data.user?.avatar || ""} name={name === NA ? "Rider" : name} size={110} />
        <div className="flex-1 space-y-2 text-[15px]">
          <p><span className="font-bold">Name:</span> <span className="text-gray-700">{name}</span></p>
          <p><span className="font-bold">Phone number:</span> <span className="text-gray-700">{phone}</span></p>
          <div className="flex items-center gap-2">
          ...
          <p><span className="font-bold">Zone:</span>{" "}
            {zone !== NA
      ...
      <div className="px-6 pb-6">
        ...
        <div className="flex gap-3 items-stretch">
          ...
          <div className="flex-1 min-w-0 rounded-xl overflow-hidden bg-white shadow-sm border border-gray-100 flex flex-col">
            <p className="text-xs font-bold text-gray-700 px-3 pt-2.5 pb-2 border-b border-gray-100">
              Most Frequent Zone: <span className="text-[#1a3f1c]">{mostFreqZone !== NA ? mostFreqZone : "—"}</span>
            </p>
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-md" style={{ backgroundColor: "#e8f7e8" }}>
      ...
      <div className="px-6 pb-4 flex items-start gap-5">
        ...
        <div className="flex-1 space-y-2 text-[15px]">
          <p><span className="font-bold">Name:</span> <span className="text-gray-700">{fullName}</span></p>
          <p><span className="font-bold">Phone number:</span> <span className="text-gray-700">{data.phoneNumber || data.phone || NA}</span></p>
          <p><span className="font-bold">Email:</span> <span className="text-gray-700">{data.email || NA}</span></p>
          <p><span className="font-bold">Address:</span> <span className="text-gray-700">{data.address || NA}</span></p>
        </div>
      </div>
      ...
      <div className="px-6 pb-6">
        <div className="rounded-xl p-4 space-y-2.5 text-[15px]" style={{ backgroundColor: "#98ef9b" }}>
          {!isHead && (
            <p>
        ...
        if (type === "staff") {
          ...
          if (!lm) {
            ...
            try {
              ...
              setLineManagerName(
                `${lmData?.firstName ?? ""} ${lmData?.lastName ?? ""}`.trim() || lmData?.email || "—"
              );
            } catch { setLineManagerName("—"); }
          }
        }
      } catch (err: any) {
